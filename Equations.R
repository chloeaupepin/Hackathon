...
model <- function(t, pop, param){
  ...
  with(as.list(c(pop, param)), {
    ...
    dS_s <- -S_s*beta_s*hs*G_s + gamma_s*R_s - mus*S_s + gsS*S_s + alpha_w_s*S_w + alpha_a_s*S_a 
    dR_s <- S_s*beta_s*hs*G_s - gamma_s*R_s - mus*R_s + gsR*R_s + alpha_w_s*R_w + alpha_a_s*R_a
    dG_s <- - beta_s*hs*G_s*S_s -musG*G_s + deltas*R_s + alpha_w_s*G_w + alpha_a_s*G_a
    
    dS_w <- -S_w*beta_w*G_w + gamma_w*R_w - muw*S_w + gwS*S_w + alpha_s_w*S_s + alpha_a_w*S_a 
    dR_w <- S_w*beta_w*G_w - gamma_w*R_w - muw*R_w + gwR*R_w + alpha_s_w*R_s + alpha_a_w*R_a
    dG_w <- - beta_w*G_w*S_w -muwG*G_w + deltaw*R_w + alpha_s_w*G_w + alpha_a_w*G_a
